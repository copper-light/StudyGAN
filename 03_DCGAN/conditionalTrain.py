...
from ConditionalCnnGAN import Discriminator, Generator, createOnehotSeed, show_plt
...
def train():
    ...
    genr_optimizer = torch.optim.Adam(generator.parameters(), lr=lr)

    def train_step(model, x, label, target, criterion, optimizer):
        model.train()
        pred = model(x, label)
    ...
    for epoch in progress:
        ...
        for step, (x, label) in enumerate(loader):
            ...
            onehot_label = torch.nn.functional.one_hot(label.to(device), num_classes).to(device)
            target = torch.ones(batch_size, 1).to(device)
            pos_loss = train_step(discriminator, x, onehot_label, target, criterion, disc_optimizer)

            seed = createOnehotSeed(label, num_classes).to(device)
            x = generator(seed).detach()
            target = torch.zeros(int(batch_size), 1).to(device)
            neg_loss = train_step(discriminator, x, onehot_label, target, criterion, disc_optimizer)

            # seed = createOnehotSeed(label.reshape(-1), num_classes).to(device)
            x = generator(seed)
            target = torch.ones(batch_size, 1).to(device)
            gen_loss = train_step(discriminator, x, onehot_label, target, criterion, genr_optimizer)
